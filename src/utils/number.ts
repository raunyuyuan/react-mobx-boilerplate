...
export const thousandsCount = (v: number | string): typeof v | string => {
  ...
  const integer = (v as string).substring(isNegative ? 1 : 0, hasPoint ? pointIdx - 1 : (v as string).length);
...
export const toFixed = (v: string | number, a: number): typeof v | string => {
  ...
  if (hasPoint) {
    ...
    const afteraNum = Number((formatV as string)[pointIdx + (a as number) + 1]);
    if (a > 0) {
      if (afteraNum >= 5) {
